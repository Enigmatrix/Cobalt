            ...
            if (self != null && to != null)
            {
                ...
                var props =(
                    ...
                    where !(selfValue == toValue ||
                            ...
                            (selfValue.GetType().GetProperties(BindingFlags.Public | BindingFlags.Instance).Count() != 0 && selfValue.GetType() != typeof(object) && selfValue.GetType() == toValue.GetType() && IsDeepEquals(selfValue, toValue, selfValue.GetType())))
                    select (selfValue, toValue, pi.Name)).ToList();
                if(props.Count == 0) return;
                throw new AggregateException(props.Select(
                    x => new Exception($"Property {x.Item3} does not match in object {self}, left: {x.Item1} | right: {x.Item2}")));
            }
