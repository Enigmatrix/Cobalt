...
#[tokio::test]
async fn add_remove_app_tag() -> Result<()> {
    let mut db = test_db().await?;
    ...
    // Test adding a tag to an app
    repo.add_app_tag(&app1.id, &tag1.id).await?;

    // Verify the tag was added by checking get_apps
    let apps = repo
        .get_apps(Times {
            day_start: 0,
            week_start: 0,
    ...
    // adding repeated tag does not error
    repo.add_app_tag(&app1.id, &tag1.id).await?;
    ...
    repo.add_app_tag(&app1.id, &tag2.id).await?;
    repo.add_app_tag(&app2.id, &tag2.id).await?;
    ...
    assert_eq!(
        from_db,
        vec![(1, vec![1, 2]), (2, vec![2]),]
            .into_iter()
            .map(|(k, v)| (Ref::new(k), RefVec(v.into_iter().map(Ref::new).collect())))
            .collect()
    );

    repo.remove_app_tag(&app1.id, &tag2.id).await?;
    ...
    repo.remove_app_tag(&app2.id, &tag2.id).await?;
    repo.remove_app_tag(&app1.id, &tag1.id).await?;
    ...
    // cannot remove tag twice, cannot remove non-existent tag.
    // no errors are thrown in these cases.
    repo.remove_app_tag(&app1.id, &tag1.id).await?;
    repo.remove_app_tag(&app1.id, &Ref::new(3)).await?;
