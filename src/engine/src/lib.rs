...
fn real_main() -> Result<()> {
    ...
    let fg = foreground_window_session(&config, &mut browser_state.blocking_write())?;
    ...
    let res: Result<()> = rt.block_on(async move {
        ...
        for attempt in 0.. {
            if attempt > 0 {
                ...
                fg = foreground_window_session(config, &mut *browser_state.write().await)?;
                now = Timestamp::now();
            }
...
fn foreground_window_session(
    config: &Config,
    browser_state: &mut web::StateInner,
